...
class Variables {
  constructor(serverless) {
    ...
    this.ssmRefSyntax = RegExp(
      /^(?:\${)?ssm(?:\.([a-zA-Z0-9-]+))?:([a-zA-Z0-9_.\-/]+)[~]?(true|false|split|decryptAsPlaintext)?/
    );
  ...
  getValueFromSsm(variableString) {
    ...
    const decrypt = groups[3] === 'true' || groups[3] === 'decryptAsPlaintext';
    const split = groups[3] === 'split';
    const decryptAsPlaintext = groups[3] === 'decryptAsPlaintext';
    return this.serverless
      .getProvider('aws')
      .request(
        'SSM',
      ...
      .then(
        response => {
          ...
          if (type !== 'StringList' && param.startsWith('/aws/reference/secretsmanager') && !decryptAsPlaintext) {
            try {
